...
async def test_single_turn_with_lesson_context():
    ...
    print(f"\n✓ {num_raising}/{len(responses)} students would participate")
...
async def test_multi_turn_with_context():
    ...
    print(f"\n✓ Loaded {len(profiles)} profiles and created orchestrator")
    ...
    for response in responses1:
        ...
        print(f"  ✋ {response.student_name}: confidence {response.confidence_score:.0%}, raised_hand={response.would_raise_hand}")
    
    # Pick a student who raised hand
    respondent1 = next((r for r in responses1 if r.would_raise_hand), responses1[0])
    print(f"\n👨‍🏫 Calls on: {respondent1.student_name}")
    print(f"💬 \"{respondent1.response}\"")
    ...
    print(f"\n📊 Turn 2 Results (with conversation context):")
    for response in responses2:
        validate_student_response(response, "multi_turn_turn2")
        print(f"  ✋ {response.student_name}: confidence {response.confidence_score:.0%}")
    ...
    print(f"\n👨‍🏫 Calls on: {respondent2.student_name}")
    print(f"💬 \"{respondent2.response}\"")
    
    print(f"\n✓ Multi-turn test completed: {len(responses1)} students responded in each turn")
...
async def test_audio_generation_with_context():
    ...
    print(f"👨‍🏫 Teacher: \"{request.prompt}\"")
    ...
    for response in responses:
        ...
        has_audio = "✅ Audio" if response.audio_base64 else "❌ No audio"
        ...
        if response.response:
            ...
            if response.audio_base64:
                ...
                try:
                    ...
                    audio_count += 1
                    print(f"  💾 Saved: {audio_file.name} ({len(audio_bytes)} bytes)")
                except Exception as e:
                    print(f"  ❌ Error: {e}")
    
    # Summary
    num_with_text = sum(1 for r in responses if r.response)
    print(f"\n✓ Generated audio for {audio_count}/{num_with_text} responses")
...
async def test_student_approach_grounding():
    ...
    print("📋 Student Approaches for this problem:")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("🎯 Responses (should reflect approaches above):")
    print("="*80)
    ...
    print(f"\n✓ All {len(responses)} students' responses are grounded in their approaches")
